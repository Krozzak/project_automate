...
**Visuels** : Carousel 3 slides — (1) contraste 3 phases 80%/60%/40% avec titre, (2) workflow Telegram → Whisper → GPT → Notion avec titre, (3) exemple entrée Notion structurée. Logo ekenor.com à ajouter manuellement en bas de chaque slide (Canva).
...
J'ai essayé de me rappeler d'un voyage d'il y a deux ans. 80% oublié.
L'année dernière, j'avais pris quelques lignes par jour. 60% oublié.
Cette année, j'ai raconté chaque soir à un bot comme je l'aurais fait à un pote. 40% oublié.

La différence entre les trois, c'est pas la mémoire — c'est comment tu captures.

Écrire quelques lignes, c'est déjà mieux que rien. Mais raconter à voix haute comme si tu t'adressais à quelqu'un — c'est un autre niveau. Tu reconstruis l'histoire, pas juste les faits.

Les Grecs le savaient déjà : la tradition orale ne transmettait pas des faits, elle racontait des histoires. Raconter, c'est ancrer. ⚓

Chaque soir, je sortais mon téléphone et je racontais ma journée. Un repas, une rencontre, un truc qui avait mal tourné. Trois minutes, parfois moins.

Derrière la note vocale, un workflow :

📱 Telegram → 🎙️ Whisper → 🤖 GPT → 📓 Notion

Chaque entrée ressort structurée automatiquement : lieu, mood, type d'activité, si c'était un moment fort.
...
**Carousel 3 slides** — histoire en 3 temps : problème → système → résultat.
**Logo** : badge ekenor.com à ajouter manuellement en bas de chaque slide via Canva (template fixe = placement identique partout).

| Slide | Titre | Contenu |
| --- | --- | --- |
| 1 | "On oublie 80% d'un voyage en une semaine" | 3 blocs progressifs 80%/60%/40%, bloc 3 mis en valeur |
| 2 | "Alors j'ai construit ça" | Workflow Telegram → Whisper → GPT → Notion |
| 3 | "Au lieu de bribes — des histoires complètes" | Carte Notion mockup : Jour 3 · Médina · Culture · ⭐ Highlight |
